...
async def create_signal_async(
    ...
    try:
        ...
        if not user_id or user_id.strip() == "":
            return "❌ User ID is required! Please login.", get_signals_table()
        ...
        if not success:
            return "❌ Failed to save to DynamoDB", get_signals_table(user_id)
...
def create_signal(
    ...
    return asyncio.run(
        create_signal_async(exchange, symbol, condition, target_price, user_id or None, notes or None, save_to_sheets)
    )
...
async def delete_signal_async(signal_id_or_full: str, current_user: str = "") -> Tuple[str, pd.DataFrame]:
    """Удаление сигнала - поддерживает как короткий ID так и полный ID из dropdown"""
    try:
        # Находим сигнал
        signals = await storage.get_all_signals()
        signal_to_delete = None

        # Если это полный ID (из dropdown)
        for signal in signals:
            if signal.id == signal_id_or_full:
                signal_to_delete = signal
                break

        # Если не нашли - пробуем по короткому ID
        if not signal_to_delete:
            for signal in signals:
                if signal.id.startswith(signal_id_or_full.replace('...', '')):
                    signal_to_delete = signal
                    break

        if not signal_to_delete:
            return f"❌ Signal not found: {signal_id_or_full}", get_signals_table(current_user)

        # Удаляем
        success = await storage.delete_signal(signal_to_delete.id)

        if success:
            return f"✅ Signal deleted: {signal_to_delete.name}", get_signals_table(current_user)
        else:
            return f"❌ Failed to delete signal", get_signals_table(current_user)

    except Exception as e:
        logger.error(f"❌ Error deleting signal: {e}")
        return f"❌ Error: {e}", get_signals_table(current_user)


def delete_signal(signal_id: str, current_user: str = ""):
    """Wrapper для удаления сигнала"""
    return asyncio.run(delete_signal_async(signal_id, current_user))
...
# ============================================================================
# ФУНКЦИЯ ДЛЯ ЗАГРУЗКИ СИГНАЛОВ В DROPDOWN
# ============================================================================
def load_user_signals_dropdown(current_user: str):
    """Загрузка сигналов текущего пользователя для dropdown"""
    try:
        signals = asyncio.run(storage.get_all_signals())

        # Фильтруем только сигналы текущего пользователя
        user_signals = [s for s in signals if s.user_id and s.user_id.strip() == current_user.strip()]

        if not user_signals:
            return gr.update(choices=[], value=None), {}

        # Формируем choices для dropdown: "Name (Symbol, Condition, $Price)" -> signal.id
        choices = []
        signal_mapping = {}
        ...
        for signal in user_signals:
            ...
            signal_mapping[label] = signal.id

        return gr.update(choices=choices, value=None), signal_mapping

    except Exception as e:
        logger.error(f"❌ Error loading signals for dropdown: {e}")
        return gr.update(choices=[], value=None), {}
...
def create_interface():
    ...
    with gr.Blocks(title="Trading Signal System with Auth", theme=gr.themes.Soft()) as app:
        ...
        signal_mapping = gr.State({})  # Mapping для dropdown: label -> signal_id
        ...
        with gr.Column(visible=False) as main_app:
            ...
            # TAB 3: DELETE SIGNAL WITH DROPDOWN
            with gr.Tab("🗑️ Delete Signal"):
                ...
                # Удаление сигнала
                delete_btn.click(
                    fn=lambda dropdown_value, mapping, user: delete_signal(mapping.get(dropdown_value, ""), user) if dropdown_value and mapping else ("❌ Please select a signal", get_signals_table(user)),
